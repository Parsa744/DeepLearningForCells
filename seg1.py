...
#files = os.listdir('/scratch1/users/hariri/Seg_Dataset1/Seg')
#for f in files:
#    print(f) 
#download the data
"""
data_folder = os.path.join(root_folder, "kaggle-data")
url = "https://gitlab.com/baigouy/models/-/raw/master/dataset_1.zip"
if not os.path.exists(data_folder):
    os.makedirs(data_folder    tmp_path = os.path.join(data_folder, "data.zip(1)")
    ...
    os.remove(tmp_path)
data_folder = os.path.join(data_folder, "dataset_1.zip")
assert os.path.exists(data_folder)
"""
#data_folder = os.path.join(root_folder, "kaggle-data")
data_folder = os.path.join(root_folder, "scratch1/users/hariri/Seg_Dataset1")

image_paths = glob("/scratch/users/hariri/Seg_Dataset1/Org/*.png")
#image_paths.sort()
# Sampleing 20% to test hiper parameters (randomly)
#image_paths = random.sample(image_paths, int(len(image_paths) * 0.2))
mask_paths = glob("/scratch1/users/hariri/Seg_Dataset1/Seg/*.png")
#mask_paths.sort()
# TODO load and normalize the test images
test_image_paths = glob("/scratch/users/hariri/Test_Dataset1/Org/*.png")
#test_image_paths.sort()
# Random sampleing
#test_image_paths =  random.sample(test_image_paths, int(len(test_image_paths) * 0.2))
test_mask_paths = glob("/scratch/users/hariri/Test_Dataset1/Seg/*.png")
#test_mask_paths.sort()

all_images = image_paths + test_image_paths
all_images.sort()
all_mask = mask_paths + test_mask_paths
all_mask.sort()

train_r = 0.8

image_paths = all_images[:int(train_r*len(all_images))]
mask_paths = all_mask[:int(train_r*len(all_images))]

test_image_paths = all_images[int(train_r*len(all_images)):]
test_mask_paths = all_mask[int(train_r*len(all_images)):]
...
unique_shapes = set([imageio.imread(i).shape for i in image_paths])
...
images = [imageio.imread(i) for i in  image_paths]
masks = [imageio.imread(m).astype("float32") for m in  mask_paths]
...
train_images = [img for i, img in enumerate(images) if i == 0 or i % 20 != 0]
train_masks = [mask for i, mask in enumerate(masks) if i == 0 or i % 20 != 0]
val_images = images[::20][1:]
val_masks = masks[::20][1:]
...
train_dataset = NucleiDataset(train_images, train_masks, mask_transform=msk_transform)
val_dataset = NucleiDataset(val_images, val_masks, mask_transform=msk_transform)
...
def train_epoch(model, loader, loss, metric, optimizer):
    ...
    for i, (x, y) in enumerate(loader):
        ...
        pred = model(x)
        print('x:', np.shape(x), 'y:', np.shape(y), 'pred:', np.shape(pred))
...
score = dice_score(y, torch.from_numpy(mask).to(device)).cpu().detach()
score
...
#test_mask_paths =  random.sample(test_mask_paths, int(len(test_mask_paths) * 0.2))
for timageOrg in test_image_paths:
	gray(timageOrg)
for timage  in test_mask_paths:
	gray(timage)

assert len(test_image_paths) == len(test_mask_paths)
imgs = [imageio.imread(i) for i in  test_image_paths]

ims_flat = np.concatenate([im.ravel() for im in imgs])
mean, std = np.mean(ims_flat), np.std(ims_flat)
test_images = [(im.astype("float32") - mean) / std for im in imgs]
test_masks = [imageio.imread(m) for m in  test_mask_paths]
print((test_images[0]))
print((test_masks[0]))
...
with torch.no_grad():
    for im, mask in zip(test_images, test_masks):
        ...
        plt.savefig('test'+str(format(int(time.time())))+'.png', dpi=600, bbox_inches='tight')
        plt.show()
        counter += 1
...
with ZipFile(file_name, 'r') as zipp:
	...
	# extracting all the files
	zipp.extractall()
reall_test = glob(os.path.join("SomeTestImages/", "*.png"))
reall_test.sort()
for i in reall_test:
  gray(i)
...
with torch.no_grad():
    for im, mask in zip(test_images, test_masks):
        ...
        ax[3].axis("off")
        ax[3].imshow(nuclei, cmap=get_random_colors(nuclei), interpolation="nearest")
        plt.savefig('hardtest'+str(format(int(time.time())))+'.png', dpi=600, bbox_inches='tight')
        plt.show()
...
# Print the runtime
print("Runtime of SomeTestImages: {:.2f} seconds".format(runtime))
